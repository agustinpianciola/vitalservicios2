...
class AccountPaymentGroup(models.Model):
    ...
    es_canal_2 = fields.Boolean( store=True, read_only=False) #compute='_compute_canal',
    pago_directo = fields.Boolean(store=True, read_only=True) #compute='_compute_canal',
    
    @api.depends('es_canal_2')
    def _compute_canal(self):
            index = 1
            for record in self.to_pay_move_line_ids:
                if index == 1:
                    if record.move_id.journal_id.es_canal_2 == True:
                        self.es_canal_2 = True
                    else:
                        self.es_canal_2 = False 
            index += 1 
            
    @api.onchange('es_canal_2')
    def dominio_pago(self):
        #if self.pop_up == False:
        #index = 1
        for rec in self:
            if rec.pago_directo==False:    
            ...
            #raise UserError (_(domain))
                return {'domain': {'to_pay_move_line_ids': domain}}
        #index += 1 

    def _get_to_pay_move_lines_domain(self):
        self.ensure_one()
        if self.es_canal_2 == True:
            return [
